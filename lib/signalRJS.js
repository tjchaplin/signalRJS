...
var longPollingConnection = require('./transports/longPollingConnection');
...
SignalRJS.prototype.connect = function(req,res){
	...
	if(req.query.transport === TRANSPORT_TYPES.longPolling)
		longPollingConnection.connect(req,res,onConnect);
};
...
SignalRJS.prototype.poll = function(req,res) {
	if(req.query.transport === TRANSPORT_TYPES.longPolling){
		...
		res.transport = req.query.transport;
		clientManager.put(connectionToken,res);
		longPollingConnection.handlePoll(req,res);
	}
};
...
SignalRJS.prototype.send = function(messageData) {		
	clientManager.getAll(function(clients){
		clients.forEach(function(client){
			...
			if(client.transport === TRANSPORT_TYPES.longPolling)
				longPollingConnection.send(client,messageData);
		});
	});
};
